...
for i=1:size(files)
    ...
    for k=1:numel(ff) % calculate indicators for each flexibility factor
        ...
        %% TODOs
        % format output and save it to a csv/excel file for each structure variant
        % dont forget ff(k), num_projects, mean(num_activities)
        % print max/original with ff=0, min,max with fr=0.1,0.2,0.3,0.4 for each instances, total 9 result per instance
