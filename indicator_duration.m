...
% example usage: >> [XDUR,VADUR] = indicator_duration(PDM, num_activities, num_modes, sim_type)
% example output: >> XDUR = 2.7333; VADUR = 2.1333
function [XDUR,VADUR] = indicator_duration(PDM, num_activities, num_modes, sim_type)

n = double(num_activities); % cast number of activities from int32 to double, to avoid truncation
...
for j=1:num_modes
    ...
    VADUR(1,j) = sum((TD(:,j) - XDUR(1,j)).^2) / (n - 1); % return variance of duration
end
